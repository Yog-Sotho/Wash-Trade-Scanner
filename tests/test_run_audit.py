...
async def test_run_audit_basic():
    ...
    with (
        patch("scripts.run_audit.Storage") as MockStorage,
        ...
        MockHeuristics.return_value.run_all_heuristics = AsyncMock(
            return_value=([], {})
        )
        ...
        await run_audit(
            chain_id=1, pool_address="0x" + "b" * 40, sync_historical=True, use_ml=False
        )
