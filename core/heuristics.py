...
class HeuristicDetector:
    ...
    async def detect_high_frequency_bot(
        ...
        for sender, sender_trades in sender_groups.items():
            ...
            # Fully vectorized with NumPy for speedup
            timestamps = np.array(
                [t.block_timestamp.timestamp() for t in sender_trades]
            )
            ...
            mean_vol = np.mean(volumes) if volumes.size > 0 else 0.0
            volume_std = np.std(volumes, ddof=1) if volumes.size > 1 else 0.0
