...
## 2026-06-15 - [Volume Anomaly Detection Micro-optimization]
**Learning:** High-throughput loops over trade objects (e.g., 100k+ trades) are sensitive to redundant object creation and attribute access. Normalizing timestamps using `.replace()` repeatedly on trades from the same block/hour creates significant overhead.
**Action:** Implement a local `bucket_cache` for normalized timestamps and reuse calculated anomaly scores instead of re-evaluating via `is_anomaly`. Resulted in ~33% speedup for the volume anomaly heuristic.
