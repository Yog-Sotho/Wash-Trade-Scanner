...
## 2025-10-05 - Inconsistent security enforcement across modules
**Vulnerability:** While the `MultiChainIngestor` enforced block range limits and chain ID verification, the `EntityClusterer` (which also performs RPC queries) lacked these protections. This left the system vulnerable to DoS and network integrity issues when using clustering features.
**Learning:** Security controls must be applied consistently to all modules that share similar risks (e.g., RPC interaction), rather than relying on a single "secure" entry point.
**Prevention:** Extract common security checks (protocol validation, chain ID verification, range limits) into shared utilities or base classes to ensure uniform enforcement across the entire codebase.
