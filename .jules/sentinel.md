...
## 2026-06-16 - Information Exposure via Stack Traces in CLI entry points
**Vulnerability:** CLI scripts (`run_audit.py`, `train_model.py`) leaked internal implementation details and file paths by using `logger.exception` in top-level handlers and failing to catch Pydantic validation errors during argument parsing.
**Learning:** Standard validation failures and unexpected runtime errors can disclose sensitive environment information if full stack traces are included in production-level logs.
**Prevention:** Explicitly catch validation errors (like `pydantic.ValidationError`) at entry points for clean error reporting. Use `logger.error` for the failure summary and move detailed stack traces to `logger.debug(..., exc_info=True)`.
